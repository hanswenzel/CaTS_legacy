...
ckm-go
    builds and runs the CaTS executable
ckm--
    just runs the CaTS executable

Objectives
------------

CaTS is really intended to feature **Minimal** usage of Opticks. 
...
* CaTS is NOT a place for debugging, the 2nd executable that 
  adopts the CaTS geocache and gensteps is where debugging is done

* moving to not using PLOG in CaTS makes a stark contrast
...
ckm-key(){ echo CaTS.X4PhysicalVolume.World.$(ckm-dig) ; }
ckm-key-export(){ export OPTICKS_KEY=$(ckm-key) ;  }
ckm-indexer-test(){   OPTICKS_KEY=$(ckm-key) IndexerTest --envkey ; }


ckm-idpath(){ echo $LOCAL_BASE/opticks/geocache/CaTS_World_g4live/g4ok_gltf/$(ckm-dig)/1 ; }
...
{ 
    ...
    CaTS  # NB the Opticks is embedded via G4OK : so commandline doesnt get thru 
}
...
Uncomment the relevant B slot then run ckm-so::
    ...
    pwd /usr/local/opticks/geocache/CaTS_World_g4live/g4ok_gltf/792496b5e2cc08bdf5258cc12e63de9f/1
...
ckm-addr2line()
{
    ...
    PATH=/usr/bin lldb $(which CaTS) -o "source list -a $addr"  --batch
}
...
ckm-- OR ckm-go  
    CaTS : geant4 example app, with genstep and photon collection
