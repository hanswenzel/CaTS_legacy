...
G4VModularPhysicsList* PhysicsConfigurator::Construct(G4String physName)
{
  ...
  //    G4String physName =
  //    PhysicsConfigurator::getInstance()->getReferencePhysicsList();
  //  if (PhysicsConfigurator::getInstance()->isEnable_OpticalConstructor())
  //  physName = physName + "+OPTICAL";
  // if (PhysicsConfigurator::getInstance()->isEnable_StepLimiter()) physName =
  // physName + "+STEPLIMIT";
  // if (PhysicsConfigurator::getInstance()->isEnable_NeutronKiller()) physName
  // = physName + "+NEUTRONLIMIT";
  if(verbose)
